...
- 🤖 AI 에이전트와 스크립트를 위한 `winC2D.exe --cli` 모드
...
1. [Releases](https://github.com/Aknirex/winC2D/releases) 에서 최신 버전 다운로드

   | 버전 | 크기 | 용도 |
   | --- | --- | --- |
   | **독립 실행형**（`-standalone.exe`） | ~70–80 MB | ⭐ 권장 — .NET 8 런타임 포함, 즉시 사용 가능 |
   | **프레임워크 종속**（`-framework-dependent.exe`） | ~10–15 MB | .NET 8 런타임이 이미 설치된 사용자용 |
...
같은 실행 파일에 `--cli` 를 붙이면 JSON을 출력합니다:
...
실제 마이그레이션에는 관리자 권한 또는 Windows Developer Mode가 필요합니다.
