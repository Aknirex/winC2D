...
- 🛡️ 起動時に自動で管理者権限を要求
- 🤖 AI エージェントやスクリプト向けの `winC2D.Cli.exe` モード
...
1. [Releases](https://github.com/Aknirex/winC2D/releases) から最新版をダウンロード
   ...
   | バージョン | サイズ | 用途 |
   | --- | --- | --- |
   | **スタンドアロン版**（`-standalone.zip`） | ~70–90 MB | ⭐ 推奨 — .NET 8 ランタイムが含まれ、すぐに使用可能 |
   | **フレームワーク依存版**（`-framework-dependent.zip`） | ~10–20 MB | .NET 8 ランタイムが既にインストールされているユーザー向け |
