...
- 🌙 深色 / 淺色主題自動跟隨系統，可手動切換
- 🛡️ 啟動時自動要求系統管理員權限
- 🤖 Agent CLI 模式：使用 `winC2D.Cli.exe` 供 AI Agent 與腳本呼叫
...
1. 前往 [Releases](https://github.com/Aknirex/winC2D/releases) 下載最新版本
   ...
   | 版本 | 大小 | 適用場景 |
   | --- | --- | --- |
   | **獨立版**（`-standalone.zip`） | ~70–90 MB | ⭐ 推薦 — 內含 .NET 8 執行時，下載即用 |
   | **依賴版**（`-framework-dependent.zip`） | ~10–20 MB | 系統已安裝 .NET 8 執行時的使用者 |
