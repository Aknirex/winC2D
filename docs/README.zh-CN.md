...
1. 前往 [Releases](https://github.com/Aknirex/winC2D/releases) 下载最新版本
   ...
   | 版本 | 大小 | 适用场景 |
   | --- | --- | --- |
   | **独立版**（`-standalone.zip`） | ~70–90 MB | ⭐ 推荐 — 内含 .NET 8 运行时，下载即用 |
   | **依赖版**（`-framework-dependent.zip`） | ~10–20 MB | 系统已安装 .NET 8 运行时的用户 |

2. 解压 zip，普通用户使用 `winC2D.App.exe`，Agent 或脚本使用 `winC2D.Cli.exe`
3. 需要迁移时按提示以**管理员身份**运行
4. 支持 Windows 10 / 11
