...
1. Скачайте последнюю версию со страницы [Releases](https://github.com/Aknirex/winC2D/releases)
   ...
   | --- | --- | --- |
   | **Автономная**（`-standalone.zip`） | ~70–90 MB | ⭐ Рекомендуется — содержит .NET 8 runtime, готова к использованию |
   | **Зависимая от платформы**（`-framework-dependent.zip`） | ~10–20 MB | Для систем с предустановленным .NET 8 runtime |
